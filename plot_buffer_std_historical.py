...
# --- Configuration ---
INPUT_DIR = "field_temporal_datasets_long"
OUTPUT_DIR = "buffer_STD"
...
def check_and_extract_ndvi(df, field_name, chips_dir="data/raw_chips"):
    """
    Checks if NDVI column contains data. If it is entirely null (common in historical 
    data due to 8-band vs 5-band sensor mismatch in original pipeline), extracts NDVI 
    on-the-fly from the 5-band RapidEye TIFF files in the raw chips folder.
    """
...
def plot_field_buffer_std(csv_path):
    ...
    for idx, row in gdf.iterrows():
        ...
        # By definition of convex hull, all points are inside or on boundary,
        # but check contains for general safety
    ...
    # Remove groups with fewer than 2 points (cannot compute STD)
    ...
    plt.title(f"NDVI Standard Deviation vs. Distance to Border (Historical)\nField: {field_name}", 
...
# Loop over all historical files
print("Processing historical fields (2010-2012)...")
csv_files = glob.glob(os.path.join(INPUT_DIR, "*_long_features.csv"))
for f in csv_files:
    plot_field_buffer_std(f)
print("Historical fields processing complete.")
